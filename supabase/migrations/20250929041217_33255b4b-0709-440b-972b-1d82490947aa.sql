-- Fix get_admin_stats aggregate ORDER BY/LIMIT issues causing 42803 errors
...
BEGIN
    ...
    -- Get recent failed songs (order and limit applied in subquery)
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', t.id,
            'title', t.title,
    ...
    -- Get 10 most recent songs (any status) (order and limit applied in subquery)
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', t.id,
            'title', t.title,
