-- Fix library overview query to match moods case-insensitively
...
BEGIN
    ...
    -- Get library overview: all genre-mood combinations with counts (case-insensitive)
    WITH all_genres AS (
        SELECT unnest(ARRAY['classical', 'country', 'edm', 'hip-hop', 'jazz', 'pop', 'rock']) AS genre
    ),
    all_moods AS (
        SELECT unnest(ARRAY['upbeat', 'chill', 'aggressive', 'emotional', 'epic', 'playful']) AS mood
    ),
    all_combinations AS (
        SELECT g.genre, m.mood
        FROM all_genres g
        CROSS JOIN all_moods m
    ),
    library_counts AS (
        SELECT 
            LOWER(s.genre) as genre,
            LOWER(s.mood) as mood,
            COUNT(*) as count
        FROM public.songs s
    ...
    SELECT jsonb_agg(
        ...
        ORDER BY ac.genre, ac.mood
    ) INTO library_overview
