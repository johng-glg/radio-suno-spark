CREATE OR REPLACE FUNCTION public.next_track(p_station uuid DEFAULT NULL, p_genres text[] DEFAULT NULL, p_mood text DEFAULT NULL, p_exclude uuid[] DEFAULT NULL)
...
BEGIN
  IF p_station IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      IF v_mood IS NULL AND cardinality(COALESCE(p_genres, '{}')) = 0 THEN
  ...
  ORDER BY
    (CASE WHEN cardinality(v_genres) = 0
            OR lower(s.genre) = ANY (SELECT lower(g) FROM unnest(v_genres) g)
          THEN 1 ELSE 0 END) DESC,
    ( (CASE WHEN v_mood IS NULL OR lower(s.mood) = v_mood THEN 1.5 ELSE 0 END)
      + LEAST(COALESCE((v_taste ->> ('genre:' || lower(s.genre)))::numeric, 0), 3)
      + LEAST(COALESCE((v_taste ->> ('mood:' || lower(s.mood)))::numeric, 0), 3)
      + random() * 12 ) DESC
  LIMIT 1;
